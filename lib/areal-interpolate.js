...
function arealInterpolate(args, cb) {
  ...
  Async.map([args.input, args.target], Shapefile.read, (err, res) => {
    ...
    args.method = args.method || 'arealWeighting';
    ...
    const Bar = new Progress('generating [:bar] :percent', {
      complete: '=',
      incomplete: ' ',
      width: 40,
    ...
    target.features.forEach((feature,i) => {
      feature.properties[args.attr] = InterpolationMethods[args.method](feature, source, args.attr);
      Bar.tick(1);
    });
