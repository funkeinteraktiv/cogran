...
function arealInterpolate(args, cb) {
  if(args.mode == 'binaryDasymetricWeighting' || args.mode == 'nClassDasymetricWeighting') {
    FileLoader.readGeoJson([args.binary], function(err, res) {
      args.binary = res[0];
      onReady(args, cb);
    });
  }
...
function getAggregationFunction(args, cb) {
  return function(err, res) {
    ...
    if(args.binary) {
      console.log(args.binary.features.length);

      args.binary.features = args.binary.features.filter(d => {
        return d.properties.binary == 1;
      });

      console.log(args.binary.features.length);

      let newSourceFeatures = [];

      args.binary.features.forEach(d => {

        source.features.forEach(s => {
          const isIntersect = Turf.intersect(s,d);

          if(isIntersect) {
            isIntersect.properties = s.properties;
            newSourceFeatures.push(isIntersect);
          }
      ...
      console.log(source.features.length);

      source = Turf.featurecollection(newSourceFeatures);
