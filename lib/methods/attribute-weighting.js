...
function populationWeighting(source, target, options, progress) {
  ...
  target.features.forEach((feature,i) => {
    Logger.info(`[populationWeighting][${options.attr}][${i}/${target.features.length * 2}]`);
    onEachFeature(feature, source, target, options);
  });

  target.features.forEach((feature,i) => {
    Logger.info(`[populationWeighting][${options.attr}][${i + target.features.length}/${target.features.length * 2}]`);
...
function onEachFeatureHannes(feature, source, target, options) {
  ...
  intersects.forEach((d,i) => {
    ...
    if(!isNaN(attributeValue)) {
      result += (Qst / sourceFeatures[i].properties["Qs"]) * sourceFeatures[i].properties[options.attr];
    }
  });
