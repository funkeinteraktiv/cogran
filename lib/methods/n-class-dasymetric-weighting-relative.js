...
function nClassDasymetricWeightingRelative(source, target, options, progress) {
	
  let s = 0;
	
  source.features.forEach(i => {
    //s += d.properties[options.attr];  
    const Relative = i.properties['Relative'];
    const Aggr = i.properties['Aggr'];
	s += Relative * Aggr / 100;
  });
