...
function nClassDasymetricWeightingRelative(source, target, options) {
  let s = 0;
  ...
  Atscm.features.forEach(d => {
	...
	d.properties.Ptx = Turf.area(d) * d.properties.Pc;
	/*console.log("Ptx: " + (Turf.area(d) * d.properties.Pc));
	console.log("------------------------------------------------------");*/
  });
...
function dissolve(fc) {
  ...
  nested.forEach((d, i) => {
    ...
    Logger.info(`[nClassDasymetricWeightingRelative][dissolve][${i}/${nested.length}]`);
...
function intersect(a, b) {
  ...
  a.features.forEach((d, i) => {
    Logger.info(`[nClassDasymetricWeightingRelative][intersect][${i}/${a.features.length}]`);
...
module.exports = nClassDasymetricWeightingRelative;